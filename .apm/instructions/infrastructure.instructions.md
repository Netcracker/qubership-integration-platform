...
```
docker-compose.yml          # base local stack: runtime-catalog, engine, sessions-management, ui-proxy, postgres, opensearch, consul
...
qip-dev.env / engine-dev.env # env files mounted into the Java service containers
nginx/
...
The base `docker-compose.yml` builds and runs the three Spring Boot services from **in-repo module dirs** via relative build contexts — `../runtime-catalog`, `../engine`, `../sessions-management` (one level up from `infrastructure/`, i.e. the monorepo root). After monorepo consolidation these are sibling directories inside this repo, not separate clones. It also stands up the shared infra they depend on: PostgreSQL (catalog + `engine_qrtz_db`/`engine_checkpoints_db` created by `init-db/init.sql`), OpenSearch (engine session recording + sessions-management storage), and Consul (config + deployment publishing). Optional brokers/caches (Kafka, RabbitMQ, Redis, Pub/Sub) are opt-in via overlay compose files for chains that use them.

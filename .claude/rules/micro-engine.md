...
`Testing-Service-Context` header. Both decide per exchange, so only a test case run is mocked; the mechanics are the
same as in `engine`, with `TestingHttpComponentCustomizerProducer` in place of the Spring configuration.

 ...
 * did not name.
 *
 * <p>Extends {@link IllegalArgumentException} so callers that already treat a refused capture as a
 ...
 * named an element not in the intent or left out a required element. An unsatisfiable one
 * describes an intent no capture can meet, so asking the generator again only spends budget on a

...
import org.qubership.integration.platform.ai.productpipeline.create.design.input.BriefFlowExtractor;
...
public class AutoUploadedSpecImportCapability implements StageCapability {
  ...
  public Multi<CapabilitySignal> execute(StageExecutionContext context) {
    ...
    if (inputDraft.isPresent() && hasCatalogBinding(inputDraft.get())) {
      ...
      LOG.infof(
          "Skipped uploaded-spec import: requirement draft already has catalog binding conversationId=%s systemId=%s specificationId=%s",
          conversationId,
          draft.catalogBinding().systemId(),
          draft.catalogBinding().specificationId());
  ...
  private CapabilitySignal.Completed succeeded(
    ...
    if (outcome != null && !hasCatalogBinding(draft)) {
      ResolvedCatalogBinding binding =
          new ResolvedCatalogBinding(
              ...
              null,
              "INTERNAL");
      updatedDraft = draft.withCatalogBinding(binding);
      LOG.infof(
          "Attached catalog binding to requirement draft conversationId=%s systemId=%s specificationId=%s",
          context.conversationId(),
          outcome.systemId(),
          outcome.specificationId());
    }
  ...
  /**
   * Rewrites uploaded-spec SERVICE_CALL facts into a catalog-bound form that {@code
   * BriefFlowExtractor} can parse reliably, and emits a catalog-binding hint for each one. The
   * imported specification is already in the runtime catalog, so a local read-only match is enough
   * to pin the integration operation id that the requirement-draft binding intentionally does not
   * carry.
   */
  private FactRewrite rewriteFactsAndHints(RequirementDraft draft) {
    ...
    ResolvedCatalogBinding binding = draft.catalogBinding();
    if (binding.systemId() == null
    ...
    for (RequirementFact call : draft.facts()) {
      ...
      Optional<BriefFlowExtractor.ServiceCallIdentity> identity =
          BriefFlowExtractor.parseServiceCall(call);
      NormalizedDesignFlow probeFlow =
          identity
              .map(
                  i ->
                      probeFlowForFact(
                          call,
                          BriefFlowExtractor.participantId(i.participantDisplayName()),
                          i.participantDisplayName(),
                          i.operationQuery()))
              .orElseGet(() -> probeFlowForFact(call, "svc", "service", call.text().trim()));
      NormalizedDesignFlow.Step probeStep =
          identity
              .map(
                  i ->
                      probeStep(
                          ...
                          BriefFlowExtractor.participantId(i.participantDisplayName()),
                          i.operationQuery()))
              .orElseGet(() -> probeStep(call, "svc", call.text().trim()));
      CatalogBindingMatcher.MatchResult match = catalogBindingMatcher.match(probeFlow, probeStep);
      LOG.infof(
          "auto-uploaded-spec-import: probing fact factId=%s parsed=%s match=%s",
          call.sourceFactId(),
          identity.map(i -> i.participantDisplayName() + " | " + i.operationQuery()).orElse("none"),
      ...
      rewrittenFacts.add(
          new RequirementFact(
              call.sourceFactId(), call.polarity(), call.kind(), call.capabilityKey(), boundText));
      hints.add(
          new ArtifactCandidate(
    ...
    return new FactRewrite(List.copyOf(hints), draft.withFacts(List.copyOf(rewrittenFacts)));
  }

  private static NormalizedDesignFlow probeFlowForFact(
    ...
    return new NormalizedDesignFlow(
        "1",
        "uploaded-spec-hint-probe",
        "uploaded-spec-hint-probe",
        "",
        new NormalizedDesignFlow.Trigger(
            "http", "client", null, null, null, List.of(call.sourceFactId())),
        List.of(
        ...
        List.of(probeStep(call, toParticipantId, operationQuery)),
        List.of(),
        List.of(),
        List.of(),
        List.of(),
  ...
  private static NormalizedDesignFlow.Step probeStep(RequirementFact call) {
    return probeStep(call, "svc", call.text().trim());
  }

  private static NormalizedDesignFlow.Step probeStep(
      RequirementFact call, String toParticipantId, String operationQuery) {
    return new NormalizedDesignFlow.Step(
        "call-1",
        "service-call",
        "client",
        toParticipantId,

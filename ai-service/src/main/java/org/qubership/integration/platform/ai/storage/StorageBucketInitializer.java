...
public class StorageBucketInitializer {
  ...
  @Inject
  StorageBucketInitializer(S3Client s3, AppConfig config) {
    this.s3 = s3;
    this.config = config;
  }
  ...
  void onStart(@Observes StartupEvent ignored) {
    ...
    try {
      ...
      LOG.warnf(
          e,
          "Storage endpoint unreachable or denied at startup (bucket=%s). Service will start"
              + " without bucket verification; fix MINIO_ENDPOINT / credentials or set"

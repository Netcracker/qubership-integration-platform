...
class AutoUploadedSpecImportCapabilityTest {
  ...
  void importsAttachmentsWhenInputDraftHasNoCatalogBinding() {
    ...
    assertEquals("sys", updated.catalogBinding().systemId());
    assertEquals("spec", updated.catalogBinding().specificationId());
    assertEquals("group", updated.catalogBinding().specificationGroupId());
    assertEquals("INTERNAL", updated.catalogBinding().systemType());
  }
  ...
  void emitsCatalogBindingHintWhenImportMatchesServiceCallFact() {
    ...
    assertEquals(fact.sourceFactId(), hint.serviceCallSourceFactId());
    RequirementDraft updated =
        (RequirementDraft)
            completed.outcome().candidates().stream()
  ...
  void emitsCatalogBindingHintsForTwoUploadedSpecs() {
    ...
    when(matcher.match(any(), any()))
        .thenAnswer(
            invocation -> {
              NormalizedDesignFlow.Step step = invocation.getArgument(1);
              String q = step.operationQuery();
  ...
  private static RequirementDraft draftWithBinding() {
    return draft()
        .withCatalogBinding(
            new ResolvedCatalogBinding("system-1", "spec-1", "group-1", "op-1"));
  }

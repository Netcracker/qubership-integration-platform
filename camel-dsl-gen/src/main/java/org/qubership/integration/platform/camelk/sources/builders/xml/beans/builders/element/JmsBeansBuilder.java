...
public class JmsBeansBuilder implements ElementBeansBuilder {
    ...
    public void build(XMLStreamWriter2 streamWriter, Element element, SourceBuilderContext context) throws Exception {
        ...
        streamWriter.writeAttribute("value", ElementUtils.getPropertyAsString(element, JMS_INITIAL_CONTEXT_FACTORY));

        streamWriter.writeEmptyElement("property");
        streamWriter.writeAttribute("key", "providerUrl");
        streamWriter.writeAttribute("value", ElementUtils.getPropertyAsString(element, JMS_PROVIDER_URL));

        streamWriter.writeEmptyElement("property");
        streamWriter.writeAttribute("key", "connectionFactoryName");
        streamWriter.writeAttribute("value", ElementUtils.getPropertyAsString(element, JMS_CONNECTION_FACTORY_NAME));

        streamWriter.writeEmptyElement("property");
        streamWriter.writeAttribute("key", "username");
        streamWriter.writeAttribute("value", ElementUtils.getPropertyAsString(element, JMS_USERNAME));

        streamWriter.writeEmptyElement("property");
        streamWriter.writeAttribute("key", "password");
        streamWriter.writeAttribute("value", ElementUtils.getPropertyAsString(element, JMS_PASSWORD));

...
public class SdsTriggerInfoBeanBuilder implements ElementBeansBuilder {
    ...
    public void build(XMLStreamWriter2 streamWriter, Element element, SourceBuilderContext context) throws Exception {
        ...
        streamWriter.writeAttribute("value", ElementUtils.getPropertyAsString(element, PROHIBIT_PARALLEL_RUN_PROP));

        streamWriter.writeEmptyElement("property");
        streamWriter.writeAttribute("key", "parallelRunTimeout");
        streamWriter.writeAttribute("value", ElementUtils.getPropertyAsString(element, PARALLEL_RUN_TIMEOUT_PROP));

        streamWriter.writeEmptyElement("property");
        streamWriter.writeAttribute("key", "cron");
        streamWriter.writeEmptyElement("value", ElementUtils.getPropertyAsString(element, CRON_PROP));

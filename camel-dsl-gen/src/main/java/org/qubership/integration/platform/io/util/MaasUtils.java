...
package org.qubership.integration.platform.io.util;
...
public class MaasUtils {
    ...
    public static ArrayList<String> getMaasParams(Element element) {
        try {
            ...
            switch (element.getType()) {
                case CamelNames.SERVICE_CALL_COMPONENT,
                    ...
                    if (environment
                                .map(ServiceEnvironment::getSourceType)
                                .map(EnvironmentSourceType.MAAS_BY_CLASSIFIER::equals)
                                .orElse(false)
            ...
            throw new TemplateInstantiationException("Failed to get MaaS state.", element, e);
        }
        return new ArrayList<>();
    }
}

...
import org.qubership.integration.platform.mapper.model.datatypes.ReferenceType;
...
public class MappingDescriptionValidator {
     ...
     * @throws MapperException when one or move required fields are not mapped
     */
    private void validateMandatoryFields(MappingDescription mappingDescription) {
        ...
        if (!pathsToMandatoryAttributes.isEmpty() && mappingDescription.getActions().isEmpty()) {
            throw new MapperException(NO_MAPPING_FOR_REQUIRED_ATTRIBUTES_ERROR_MESSAGE);
        }
        ...
        if (!pathsToMandatoryAttributes.isEmpty()) {
            ...
            if (!missingMandatoryAttributes.isEmpty()) {
                throw new MapperException(MANDATORY_FIELDS_MISSING_IN_MAPPING_ERROR_MESSAGE);
            }
        }
    }

...
practice means `service-call`, `http-sender` and `graphql-sender`. The interceptor points scheme and authority at the
testing service, rewrites the request target to `/api/v1/endpoint-mocks/call`, and attaches a `Testing-Service-Context`
header.
...
Mocks bind to the design-time id and the lookup keys on `(chainId, elementId)` alone, so reading the wrong one matches
nothing after the next deploy, silently. There is no request-path property to reach for either: `ChainProperties.PATH`
...
takes `EndpointInfo` — so the implementations are duplicated rather than shared, as the rest of these two modules are.
The wire format is a single contract, though: a change to the header has to land in `micro-engine`'s copy of
...
- A test run is linked to a session through `external-session-cip-id`. The testing service generates that id, sends it
  ...
  `GET /v1/sessions/{id}`, which answers `404`; match it against the external id instead. And a session exists only when
...
- `httpcomponents_httpclient_request_seconds` keeps `chain_id`, `chain_name`, `element_id`, `element_name` and the
  ...
  than from the request. But `target_host`, `target_port` and `target_scheme` follow the route and read the testing
  service's address: the mock interceptor rewrites scheme and authority before the metrics interceptor runs.

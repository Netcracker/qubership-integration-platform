...
@UriParams
public class KafkaCustomConfiguration implements Cloneable, HeaderFilterStrategyAware {
    ...
    @UriParam(label = "consumer", defaultValue = "EVENTUAL", enums = "EVENTUAL,GUARANTEE_CONSUMPTION")
    private String consumerConsistencyMode = ConsumerConsistencyMode.EVENTUAL.toString();
    ...
    public KafkaCustomConfiguration() {
    }

    /**
     * Returns a copy of this configuration
     */
    public KafkaCustomConfiguration copy() {
        try {
            KafkaCustomConfiguration copy = (KafkaCustomConfiguration) clone();
    ...
    /**
     * Consistency mode for blue-green consumer.
     * EVENTUAL – consumers offset for candidate during promotion stays the same.
     * GUARANTEE_CONSUMPTION - consumers offset for candidate during promotion is copied from
     * the lowest offset for this group of consumers.
     */
    public void setConsumerConsistencyMode(String consumerConsistencyMode) {
        this.consumerConsistencyMode = consumerConsistencyMode;
    }

    public String getConsumerConsistencyMode() {
     ...
     * result, <code>read_committed</code> consumers will not be able to read up to the high watermark when there are in

...
public class GraphQLSessionLoggingProcessor implements Processor {
    ...
    public GraphQLSessionLoggingProcessor(CamelDebuggerPropertiesService propertiesService) {
    ...
    private boolean isPayloadLoggingEnabled(Exchange exchange) {
        if (!(exchange.getContext().getDebugger() instanceof CamelDebugger camelDebugger)) {
            return false;
        }
    ...
    private static boolean isBodyLoggingEnabled(DeploymentRuntimeProperties runtimeProperties) {

...
---
### "Endpoint" Tab
**"Endpoint"** tab is responsible for choosing AsyncAPI specification and its operations (task). Only services with AsyncAPI specification(s) in **Services** repository are available for selection.
...
> ℹ️ **Note:**
> When message processing is slow, it could lead to interruptions and other negative effects, such as repeated message processing from the topic. In such cases, it must be considered to:
...
| queues                    | M         | String    | Name of the queue(s), from where trigger will consume messages. Use comma as a separator when specifying multiple queues.                                                                                           | cip-maas-queue-1   |

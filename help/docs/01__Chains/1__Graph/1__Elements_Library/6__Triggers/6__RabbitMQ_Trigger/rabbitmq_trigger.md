...
| Addresses     | M         | String    | Address of target AMQP server. `server1:12345`                                                                                                                          | localhost:15672 |
...
| Routing Key                | O         | String    | The routing key to bind the consumer queue to the exchange with. Applied only when **Declare Topology** is on, since it describes a binding the chain would create.                                                      | rkey1         |
...
| Declare Topology          | O         | Boolean   | Whether the chain declares the exchange, the queue, their binding and the dead letter entities below when it starts.<br><br>Leave it off where the broker topology is managed elsewhere — through MaaS, or by hand — and the chain only consumes from what already exists. Turn it on to own the topology from here.<br><br>**The four Dead Letter properties, Exchange Type and Routing Key are applied only when the entities are declared, and have no effect otherwise.**<br><br>A queue that already exists with different properties is not modified: the broker rejects the declaration and the chain fails to deploy, naming the property that differs.<br>**Default value:** false | false                  |
| Exchange Type             | M         | List      | The exchange type to declare the exchange with, such as direct or topic. Applied only when **Declare Topology** is on. Enum values: <ul><li>direct</li><li>fanout</li><li>headers</li><li>topic</li></ul>**Default value:** direct                                                                                                                                                          | direct                 |
| Dead Letter Exchange      | O         | String    | The name of the dead letter exchange, to where rejected messages are routed. Applied only when **Declare Topology** is on: dead lettering is an argument of the queue, set when the queue is declared.                                                                                                                                                                                                                                                                                                   | x-dead-letter-exchange |
| Dead Letter Exchange Type | O         | List      | The type to declare the dead letter exchange with. Applied only when **Declare Topology** is on. Enum values:<ul><li>direct</li><li>fanout</li><li>headers</li><li>topic</li></ul>**Default value:** direct                                                                                                                                                                                                                                           | direct                 |
| Dead Letter Queue         | O         | String    | The name of the dead letter queue, to where rejected messages are routed. Applied only when **Declare Topology** is on.                                                                                                                                                                                                                                                                                                      | basic.queue            |
| Dead Letter Routing Key   | O         | String    | The routing key to bind the dead letter queue to the dead letter exchange with. Applied only when **Declare Topology** is on.                                                                                                                                                                                                                                                                                              | dlrkey1                |
...
| Action on duplicate           | M         | List      | Defines action that should be performed in case of key duplication detected. Possible values:<ul><li>*Ignore* - stop session and do not process the call</li><li>*Throw exception* - raise exception and stop call processing</li><li>*Execute subchain* - call selected chain via Chain Trigger. In case this option has been chosen, two more parameters (below in the table) becomes visible and editable.</li></ul> | Throw exception               |

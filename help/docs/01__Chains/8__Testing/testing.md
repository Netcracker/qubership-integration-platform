...
> ℹ️ **Note**: The **Testing** tab appears only where the testing service is deployed, reachable and reporting a non-production mode. On a production installation the tab is hidden, the **Testing** group under [Admin Tools](../../03__Admin_Tools/9__Testing/testing.md) is hidden with it, and a direct link to a testing address lands on the "not found" page. Testing is a development and verification feature; it is not intended for production installations.
...
- **Readiness** - **_Ready_** when the test case has a trigger and request settings, **_Incomplete_** otherwise.
...
- **Search field** - filters the loaded rows by the text entered.
...
Rows are loaded page by page as the table is scrolled. When some rows are still unloaded, the selection menu offers **Select all that match the filters**: bulk actions then resolve their targets on the server under the current filters rather than from the loaded rows, and the confirmation says so.

Click anywhere in a row except the name to open the **Test Case Details** side panel, which repeats the fields above in read-only form.
...
The table lists the endpoint mocks of the chain. Most columns repeat the test case ones; the mock-specific ones are:

- **Element** - the element whose outgoing call the mock answers, a clickable reference to the chain [graph](../1__Graph/graph.md).
...
The toolbar above the table adds a rule, deletes the checked rules, and enables or disables them in bulk. A local search filters the rows already in the table.
...
### Run Test Cases
Select test cases in the table and click **Run selected test cases**. The service creates a **test run** over them and reports it with a notification linking to the run.
...
- **Errors** - number of validation errors recorded, a clickable reference to them.
...
- **Export selected test case runs** - downloads the checked case runs as a file.
...
### Validation Errors
Click the **Id** of a case run, or its **Errors** count, to open the validation errors of that case run. The table lists one row per rule that did not hold:

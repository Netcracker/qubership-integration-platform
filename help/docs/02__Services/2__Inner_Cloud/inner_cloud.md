...
---
Cloud Integration Platform is able to integrate with Inner Cloud Services, that are located within the environment. Unlike for external services, Cloud Integration Platform might perform a lookup in order to fetch all Inner Services available within the Cloud.

## Process Initialization

---

- In CIP UI: to reach the Inner Cloud Services management window, please click "**Services**" on the top of the Cloud Integration Platform screen and then go to **"Inner Cloud"** tab.
...
Any inner cloud service created using VS Code Extension appears under "Services" folder. This folder can be located by expanding "CIP" folder in the left bottom.
...
- In CIP UI: for any Inner Cloud services, the service parameters and configuration is stored in CIP Catalog database. Please refer to [CIP Architecture](../../05__Architecture/cip_architecture.md) for common data storage overview.

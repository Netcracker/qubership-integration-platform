...
public final class TemplateService {
    ...
    public String applyTemplate(Element element) {
        ...
        if (template == null) {
            throw new TemplateInstantiationException("Element is not supposed to be outside a parent container.", element);
        }
        ...
        try {
            ...
            if (e.getCause() instanceof TemplateInstantiationException instantiationException) {
                if (instantiationException.getElement().isEmpty()) {
                    throw new TemplateInstantiationException(instantiationException.getMessage(), element, e);
                } else {
                    throw instantiationException;
                }
            }
            throw new TemplateInstantiationException("Fields are not properly defined or require mandatory connection", element, e);
        }
    ...
    public Template getTemplate(Element element) {
        return libraryService.lookupElementDescriptor(element.getType())
                    ...
                    .orElse(false)
                ? getTemplate(element.getType() + (element.getInputConnections().isEmpty() && element.getParent().isEmpty()

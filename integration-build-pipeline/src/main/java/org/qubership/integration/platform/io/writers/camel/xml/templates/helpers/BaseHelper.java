...
import static org.qubership.integration.platform.library.constants.CamelOptions.TOPICS;
...
public class BaseHelper {
    ...
    public Object getPropertyValue(String propertyName, Options options) {
        ...
        if (context instanceof Element element) {
            ...
            if (isMaasParamProperty(propertyName, elementType, element)) {
                return MaasUtils.getMaasParamPlaceholder(element.getOriginalId().orElse(element.getId()), propertyName);
            }
            return element.getProperties().get(propertyName);
    ...
    private static boolean isMaasParamProperty(String propertyName, String elementType, Element element) {
        ...
        if ((isTopicProperty(propertyName, elementType) && isKafkaTrigger2OrKafkaSender2Component(elementType))
            ...
            return !maasParamList.isEmpty();
        }
        return false;
    }

    private static boolean isOperationProtocolTypeKafka(String operationProtocolType) {
        return StringUtils.isNotEmpty(operationProtocolType)
            && StringUtils.equalsIgnoreCase(operationProtocolType, CamelNames.OPERATION_PROTOCOL_TYPE_KAFKA);
    }

...
public class GenerateURLHelper {
     ...
     * @return
     */
    public CharSequence integeratePathAndQueryParams(Element element) {
     ...
     * */
    private String getOldFormatUrl(Element element) {
        String baseURL = ElementUtils.getPropertyAsString(element, CamelOptions.OPERATION_PATH);
        Map<String, String> pathMap = (LinkedHashMap) element.getProperties().get(CamelOptions.OPERATION_PATH_PARAMETERS);
        Map<String, String> queryMap = (LinkedHashMap) element.getProperties().get(CamelOptions.OPERATION_QUERY_PARAMETERS);
    ...
    private String generatePathParamString(Element element) {
        String baseURL = ElementUtils.getPropertyAsString(element, CamelOptions.OPERATION_PATH);
        ...
        Map<String, String> map = (LinkedHashMap) element.getProperties().get(CamelOptions.OPERATION_PATH_PARAMETERS);
        ...
        for (String key : map.keySet()) {
            if (StringUtils.isEmpty(map.get(key))) {
                throw new TemplateInstantiationException(CHECK_REQUIRED_FIELDS_FOR_ELEMENT, element);
            }
    ...
    private String generateQueryParamString(Element element) {
        StringBuilder result = new StringBuilder();
        Map<String, String> map = (LinkedHashMap) element.getProperties().get(CamelOptions.OPERATION_QUERY_PARAMETERS);
    ...
    private boolean shouldSkipEmptyQueryParams(Element element) {
        Object skipEmptyParamsProperty = element.getProperties().get(CamelOptions.SKIP_EMPTY_QUERY_PARAMS);

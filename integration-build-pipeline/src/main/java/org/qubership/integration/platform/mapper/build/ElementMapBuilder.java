...
public class ElementMapBuilder {
    public record ElementContext(Map<String, org.qubership.integration.platform.mapper.model.datatypes.TypeDefinition> definitionMap) {}
    ...
    public Map<String, ElementWithContext> buildElementMap(org.qubership.integration.platform.mapper.model.datatypes.DataType type) {
        return buildElementMap(type, Collections.emptyMap());
    }

    public Map<String, ElementWithContext> buildElementMap(
            org.qubership.integration.platform.mapper.model.datatypes.DataType type,
    ...
    private Stream<ElementWithContext> getAttributes(
        ...
        Map<String, org.qubership.integration.platform.mapper.model.datatypes.TypeDefinition> definitions = DataTypeUtils.updateDefinitionMapFromType(
                definitionMap, dataType);
        if (dataType instanceof org.qubership.integration.platform.mapper.model.datatypes.ArrayType type) {
            return getAttributes(type.getItemType(), definitions);
        } else if (dataType instanceof org.qubership.integration.platform.mapper.model.datatypes.ObjectType type) {
            return type.getSchema().getAttributes().stream().map(attribute ->
                    new ElementWithContext(attribute, new ElementContext(definitions)));
        } else if (dataType instanceof org.qubership.integration.platform.mapper.model.datatypes.CompoundType type) {

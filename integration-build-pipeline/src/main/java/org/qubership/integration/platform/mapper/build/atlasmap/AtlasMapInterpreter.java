...
package org.qubership.integration.platform.mapper.build.atlasmap;
...
import org.qubership.integration.platform.mapper.build.MapperException;
import org.qubership.integration.platform.mapper.build.MappingInterpretation;
import org.qubership.integration.platform.mapper.build.atlasmap.xml.XmlTemplateBuilder;
import org.qubership.integration.platform.mapper.build.expressions.FieldKind;
import org.qubership.integration.platform.mapper.build.expressions.ToAtlasMapExpressionConverter;
import org.qubership.integration.platform.mapper.build.metadata.DataFormat;
import org.qubership.integration.platform.mapper.build.metadata.MetadataUtils;
import org.qubership.integration.platform.mapper.model.atlasmap.action.QIPDefaultValueAction;
import org.qubership.integration.platform.mapper.model.atlasmap.action.QIPDictionaryAction;
import org.qubership.integration.platform.mapper.model.atlasmap.action.QIPFormatDateTimeAction;
import org.qubership.integration.platform.mapper.model.atlasmap.characteristic.PropertyCharacteristics;
import org.qubership.integration.platform.mapper.model.datatypes.TypeKind;
import org.qubership.integration.platform.mapper.model.datatypes.ValueExtractor;
import org.qubership.integration.platform.mapper.model.mapping.MappingDescription;
import org.qubership.integration.platform.mapper.model.mapping.action.TransformationType;
import org.qubership.integration.platform.mapper.model.mapping.definition.Attribute;
import org.qubership.integration.platform.mapper.model.mapping.definition.Element;
...
public class AtlasMapInterpreter implements org.qubership.integration.platform.mapper.build.MappingInterpreter {
    ...
    private void fillAdditionalDataSources(MappingDescription mappingDescription, AtlasMapping atlasMapping) {
        if (!mappingDescription.getSource().getProperties().isEmpty()) {
            mappingDescription
                    ...
                    .getProperties()
                    .stream()
                    .filter(property -> property.getType() instanceof org.qubership.integration.platform.mapper.model.datatypes.ComplexType)
        ...
        mappingDescription.getConstants().stream()
                .filter(constant -> constant.getValueSupplier().getKind().equals(org.qubership.integration.platform.mapper.model.mapping.definition.constant.SupplierKind.GENERATED))
    ...
    private DataSource createDataSourceForConstantWithGeneratedValue(
            org.qubership.integration.platform.mapper.model.mapping.definition.constant.Constant constant
    ) {
        ...
        org.qubership.integration.platform.mapper.model.mapping.definition.constant.ValueGenerator generator = ((org.qubership.integration.platform.mapper.model.mapping.definition.constant.GeneratedValue) constant.getValueSupplier()).getGenerator();
        dataSource.setUri(buildGeneratedValueUri(constant.getId(), generator));
        return dataSource;
    }

    private String buildGeneratedValueUri(String id, org.qubership.integration.platform.mapper.model.mapping.definition.constant.ValueGenerator generator) {
    ...
    private void fillPropertyDataSources(Attribute property, DataSourceType direction, AtlasMapping atlasMapping) {
        org.qubership.integration.platform.mapper.model.datatypes.DataType body = switch (property.getType().getKind()) {
            case OBJECT -> property.getType();
            case ARRAY -> ((org.qubership.integration.platform.mapper.model.datatypes.ArrayType) property.getType()).getItemType();
            default -> new org.qubership.integration.platform.mapper.model.datatypes.NullType(null);
        };
    ...
    private DataSource getDataSource(String dataSourceName, MessageSchema dataSourceSchema, DataSourceType dataSourceType, Map<String, String> parameters) {
        ...
        if (dataSourceSchema.getBody() instanceof org.qubership.integration.platform.mapper.model.datatypes.ComplexType body) {
        ...
        switch (dataFormat) {
            ...
            case XML -> {
                ...
                try {
                    ...
                    throw new MapperException("Failed to create XML document template", exception);
                }
                dataSource = xmlDataSource;

            }
            default -> throw new MapperException(INCORRECT_DATA_SOURCE_FORMAT_ERROR_MESSAGE.concat(dataSourceType.value()));
        }
    ...
    private void fillConstants(MappingDescription mappingDescription, AtlasMapping atlasMapping) {
        List<Constant> constantList = mappingDescription
                .getConstants()
                .stream()
                .filter(constant -> constant.getValueSupplier().getKind().equals(org.qubership.integration.platform.mapper.model.mapping.definition.constant.SupplierKind.GIVEN))
    ...
    private Constant getConstant(org.qubership.integration.platform.mapper.model.mapping.definition.constant.Constant constant) {
    ...
    private String getConstantValue(org.qubership.integration.platform.mapper.model.mapping.definition.constant.Constant constant) {
        org.qubership.integration.platform.mapper.model.mapping.definition.constant.ValueSupplier valueSupplier = constant.getValueSupplier();
        return org.qubership.integration.platform.mapper.model.mapping.definition.constant.SupplierKind.GIVEN.equals(valueSupplier.getKind())
                ? ((org.qubership.integration.platform.mapper.model.mapping.definition.constant.GivenValue) valueSupplier).getValue() : constant.getName();
    }
    ...
    private List<Property> getProperties(Collection<Attribute> properties, String scope, DataSourceType dataSourceType) {
        return properties
                .stream()
                .filter(attribute -> !(attribute.getType() instanceof org.qubership.integration.platform.mapper.model.datatypes.ComplexType))
    ...
    private Map<String, org.qubership.integration.platform.mapper.build.ElementMapBuilder.ElementWithContext> buildElementMap(
            MessageSchema messageSchema,
            Collection<org.qubership.integration.platform.mapper.model.mapping.definition.constant.Constant> constants
    ) {
        ...
        Map<String, org.qubership.integration.platform.mapper.build.ElementMapBuilder.ElementWithContext> result = new HashMap<>();
        Stream.of(messageSchema.getHeaders(), messageSchema.getProperties())
                .map(attributes -> new org.qubership.integration.platform.mapper.model.datatypes.ObjectType(new ObjectSchema("", List.copyOf(attributes), null), null, null))
                .map(elementMapBuilder::buildElementMap)
                .forEach(result::putAll);
        result.putAll(elementMapBuilder.buildElementMap(messageSchema.getBody()));
        constants.forEach(constant -> result.put(constant.getId(), new org.qubership.integration.platform.mapper.build.ElementMapBuilder.ElementWithContext(
                constant, new org.qubership.integration.platform.mapper.build.ElementMapBuilder.ElementContext(Collections.emptyMap()))));
        return result;
    }

    private List<? extends BaseMapping> getMappings(
            MappingDescription mappingDescription,
            org.qubership.integration.platform.mapper.model.mapping.action.MappingAction action,
            Map<String, org.qubership.integration.platform.mapper.build.ElementMapBuilder.ElementWithContext> sourceElementMap,
            Map<String, org.qubership.integration.platform.mapper.build.ElementMapBuilder.ElementWithContext> targetElementMap
    ) {
    ...
    private Field buildAtlasField(
            org.qubership.integration.platform.mapper.model.mapping.action.ElementReference elementReference,
            MessageSchema messageSchema,
            Map<String, org.qubership.integration.platform.mapper.build.ElementMapBuilder.ElementWithContext> elementMap,
            boolean isSource
    ) {
        return switch (elementReference.getType()) {
            case CONSTANT -> buildConstantField(elementReference, elementMap);
            case ATTRIBUTE -> {
                org.qubership.integration.platform.mapper.model.mapping.action.AttributeReference attributeReference = (org.qubership.integration.platform.mapper.model.mapping.action.AttributeReference) elementReference;
    ...
    private Field buildHeaderField(
            org.qubership.integration.platform.mapper.model.mapping.action.AttributeReference attributeReference,
            Map<String, org.qubership.integration.platform.mapper.build.ElementMapBuilder.ElementWithContext> elementMap
    ) {
        String headerId = attributeReference.getPath().stream().findFirst().orElseThrow(
                () -> new MapperException(ATTRIBUTE_REFERENCE_PATH_IS_EMPTY_ERROR_MESSAGE));
        var attributeWithContext = elementMap.get(headerId);
        if (isNull(attributeWithContext)) {
            throw new MapperException(MAPPING_CREATION_ERROR_MESSAGE
    ...
    private Field buildPropertyField(
        ...
        org.qubership.integration.platform.mapper.model.datatypes.DataType rootType = DataTypeUtils.resolveType(root.getType(), Collections.emptyMap()).type();
        if (rootType instanceof org.qubership.integration.platform.mapper.model.datatypes.ComplexType) {
            if (path.size() == 1) {
                ...
                return buildAttributeField(
                        new org.qubership.integration.platform.mapper.model.mapping.action.AttributeReference(
            ...
            throw new MapperException("Path length for scalar attribute is greater than 1");
        }
    }
    ...
    private Field buildBodyField(
        ...
        org.qubership.integration.platform.mapper.model.datatypes.DataType rootType = (
                (bodyTypeResolveResult.type() instanceof org.qubership.integration.platform.mapper.model.datatypes.CompoundType compoundType)
    ...
    private Field buildAttributeField(
        ...
        if (path.isEmpty()) {
            throw new MapperException(ATTRIBUTE_REFERENCE_PATH_IS_EMPTY_ERROR_MESSAGE);
        }
        ...
        Map<String, org.qubership.integration.platform.mapper.model.datatypes.TypeDefinition> definitionMap = elementMap.get(lastAttribute.getId()).context().definitionMap();
        DataTypeUtils.ResolveResult typeResolveResult = DataTypeUtils.resolveType(lastAttribute.getType(), definitionMap);

        org.qubership.integration.platform.mapper.model.datatypes.DataType dataType = typeResolveResult.type();
        ...
        if (isArray) {
            DataTypeUtils.ResolveResult itemTypeResolveResult = DataTypeUtils.resolveType(
                    ((org.qubership.integration.platform.mapper.model.datatypes.ArrayType) dataType).getItemType(), typeResolveResult.definitionMap());
            dataType = itemTypeResolveResult.type();
        }

        Field field;
        if (dataType instanceof org.qubership.integration.platform.mapper.model.datatypes.ArrayType) {
            field = new ComplexField();
            field.setName(fieldName);
            field.setPath(fieldPath);
            field.setFieldType(FieldType.ANY);
        }
        if (dataType instanceof org.qubership.integration.platform.mapper.model.datatypes.CompoundType) {
            // TODO merge fields of nesting types
            field = new ComplexField();
            field.setName(fieldName);
            field.setPath(fieldPath);
            field.setFieldType(FieldType.ANY);
        } else if (dataType instanceof org.qubership.integration.platform.mapper.model.datatypes.ObjectType objectType) {
            List<Field> childFields = objectType.getSchema().getAttributes().stream()
                    .map(attribute -> new org.qubership.integration.platform.mapper.model.mapping.action.AttributeReference(
                            attributeReference.getKind(),
            ...
            switch (dataFormat) {
                ...
                default -> throw new MapperException(MAPPING_CREATION_ERROR_MESSAGE
        ...
        return path.stream()
                .map(
                        id -> Optional.ofNullable(elementMap.get(id))
                                .map(org.qubership.integration.platform.mapper.build.ElementMapBuilder.ElementWithContext::element)
                                .orElseThrow(() ->
                                        new MapperException(MAPPING_CREATION_ERROR_MESSAGE
    ...
    private String buildAtlasMapFieldPath(
            List<Element> path,
            Map<String, org.qubership.integration.platform.mapper.build.ElementMapBuilder.ElementWithContext> attributeMap
    ) {
        return path.stream()
                .map(attribute -> {
                    var defaultAttributeContextInfo = new org.qubership.integration.platform.mapper.build.ElementMapBuilder.ElementWithContext(
                            attribute, new org.qubership.integration.platform.mapper.build.ElementMapBuilder.ElementContext(Collections.emptyMap()));
    ...
    private Field buildConstantField(
        ...
        org.qubership.integration.platform.mapper.model.mapping.action.ConstantReference constantReference = (org.qubership.integration.platform.mapper.model.mapping.action.ConstantReference) elementReference;
        ...
        org.qubership.integration.platform.mapper.model.mapping.definition.constant.Constant constant =
                (org.qubership.integration.platform.mapper.model.mapping.definition.constant.Constant)
                        attributeWithContext.element();

        if (constant.getValueSupplier().getKind().equals(org.qubership.integration.platform.mapper.model.mapping.definition.constant.SupplierKind.GENERATED)) {
            ...
            field.setFieldType(dataTypeToFieldTypeConverter.convert(constant.getType()));
            field.setValue(((org.qubership.integration.platform.mapper.model.mapping.definition.constant.GivenValue) constant.getValueSupplier()).getValue());
            field.setPath("/".concat(constant.getName()));
            field.setCollectionType(CollectionType.NONE);
    ...
    private BaseMapping processTransformation(
        ...
        org.qubership.integration.platform.mapper.model.mapping.action.Transformation transformation = action.getTransformation();
        ...
        if (mapping instanceof Mapping) {
            ...
            switch (transformationType) {
                ...
                case DICTIONARY -> {
                    ...
                    Map<String, String> lookupTable = transformation.getParameters()
                            .stream()
                            .skip(1)
                            .map(org.qubership.integration.platform.mapper.build.DictionaryEntryParser::parse)
                ...
                case TRIM -> {
                    List<String> params = transformation.getParameters();
                    if (params.size() != 1) {
                        throw new MapperException("Wrong number of parameters for 'trim' transformation.");
                    }
                    Action trimAction = switch (params.getFirst()) {
                ...
                case REPLACE_ALL -> {
                    List<String> params = transformation.getParameters();
                    if (params.size() != 2) {
                        throw new MapperException("Wrong number of parameters for 'replaceAll' transformation.");
                    }
            ...
            processedMapping.getInputField().getFirst().setActions(actions);
        }
    ...
    private void validateForSupportedMappings(List<Field> inputFields, Field outputField, org.qubership.integration.platform.mapper.model.mapping.action.MappingAction action) {

        if (inputFields.isEmpty() || outputField == null) {
            throw new MapperException(MAPPING_CREATION_ERROR_MESSAGE.concat(EMPTY_FIELDS_ERROR_MESSAGE));
        }
        ...
        if (
            ...
            throw new MapperException(MAPPING_CREATION_ERROR_MESSAGE.concat(COMBINE_ARRAY_AND_PRIMITIVE_INTO_ARRAY_ERROR_MESSAGE));
        }
        ...
        if (
            ...
            throw new MapperException(MAPPING_CREATION_ERROR_MESSAGE.concat(COMBINE_SEVERAL_ARRAYS_ERROR_MESSAGE));
        }
        ...
        if (inputFields.size() > 1 && action.getTransformation() == null && !outputField.getCollectionType().equals(CollectionType.ARRAY)) {
            throw new MapperException(MAPPING_CREATION_ERROR_MESSAGE.concat(MULTIPLE_FIELDS_AGGREGATION_ERROR_MESSAGE));
        }
    }
}

...
| LOG_FILEDS_MAX_SIZE                 | 4096                                                 | Defines the maximum character length for fields: `exchange_properties`, `exchange_headers`, `exchange_body`. If not specified - the fields will not be truncated. <br><br> **Example (with value `5`):** <br> **Before**: Session COMPLETED NORMALLY. Duration 32ms. Headers: qubership, body: qubership, exchange properties: qubership <br> **After**: Session COMPLETED NORMALLY. Duration 32ms. Headers: quber..., body: quber..., exchange properties: quber...                                                                                                             |
...
| TESTING_SERVICE_ENABLED             | false                                                | Enables endpoint mocking. Turn it on only where the Testing Service is deployed: with it on, an outbound HTTP call with no matching mock gets a 404 instead of reaching the real endpoint. Changing the value requires an engine restart; chains need no redeploy. |

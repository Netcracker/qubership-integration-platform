...
class RouteUnregisterActionTest {
    ...
    @Mock
    private CamelEvent.RouteRemovedEvent anotherEvent;
    ...
    @Test
    void shouldUnregisterRoutesWhenDeploymentIsNotUnregisteredAndServiceIsAvailable() throws Exception {
        RouteUnregisterAction action = actionWithRouteRegistrationService();
        ...
        try (MockedStatic<MetadataUtil> metadataUtil = mockStatic(MetadataUtil.class)) {
            ...
            verify(routeRegistrationService).unregisterRoutes(routeRegistrationInfos);
        }
    }

    @Test
    void shouldSkipRouteDeregistrationWhenServiceIsNotAvailable() throws Exception {
        RouteUnregisterAction action = actionWithoutRouteRegistrationService();
    ...
    @Test
    void shouldUnregisterRoutesOnlyOnceWhenSameDeploymentIsProcessedTwice() throws Exception {
        RouteUnregisterAction action = actionWithRouteRegistrationService();
        Collection<RouteRegistrationInfo> routeRegistrationInfos = List.of(routeRegistrationInfo);
        ...
        try (MockedStatic<MetadataUtil> metadataUtil = mockStatic(MetadataUtil.class)) {
            ...
            verify(routeRegistrationService, times(1)).unregisterRoutes(routeRegistrationInfos);
    ...
    @Test
    void shouldUnregisterRoutesForDifferentDeploymentsIndependently() throws Exception {
        RouteUnregisterAction action = actionWithRouteRegistrationService();
        Collection<RouteRegistrationInfo> routeRegistrationInfos = List.of(routeRegistrationInfo);
        ...
        try (MockedStatic<MetadataUtil> metadataUtil = mockStatic(MetadataUtil.class)) {
            ...
            verify(routeRegistrationService).unregisterRoutes(routeRegistrationInfos);
            verify(routeRegistrationService).unregisterRoutes(anotherRouteRegistrationInfos);
        }
    }

    @Test
    void shouldNotUnregisterRoutesAgainWhenServiceIsUnavailableAndSameDeploymentIsProcessedTwice() throws Exception {
        RouteUnregisterAction action = actionWithoutRouteRegistrationService();
    ...
    @Test
    void shouldPropagateExceptionWhenRouteDeregistrationFails() {
        RouteUnregisterAction action = actionWithRouteRegistrationService();
        Collection<RouteRegistrationInfo> routeRegistrationInfos = List.of(routeRegistrationInfo);
        RuntimeException exception = new RuntimeException("Failed to unregister routes");

        stubDeploymentWithSnapshot(deploymentInfo, DEPLOYMENT_ID, snapshotInfo, SNAPSHOT_ID);
        when(route.getCamelContext()).thenReturn(camelContext);
        org.mockito.Mockito.doThrow(exception)
            .when(routeRegistrationService)
            .unregisterRoutes(routeRegistrationInfos);
    ...
    private RouteUnregisterAction actionWithRouteRegistrationService() {
        try (MockedStatic<InjectUtil> injectUtil = mockStatic(InjectUtil.class)) {
            ...
            return new RouteUnregisterAction(routeRegistrationServiceInstance);
        }
    }

    private RouteUnregisterAction actionWithoutRouteRegistrationService() {
        try (MockedStatic<InjectUtil> injectUtil = mockStatic(InjectUtil.class)) {
            injectUtil.when(() -> InjectUtil.injectOptional(routeRegistrationServiceInstance))

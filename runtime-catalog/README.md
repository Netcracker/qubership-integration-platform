...
| QIP_EXPORT_LEGACY_FORMAT          | false                                                | Export configuration in the pre-#553 "old" format: flat `service-<id>.yaml` names carrying `content.integrationSystemType`. Set it when the archive has to be imported by an older Runtime Catalog — the current format states the service type in the file name, and older versions do not discover those names. |
| `*_JSON_SCHEMA_URI`               | `http://qubership.org/schemas/product/qip/<entity>.schema.yaml` | The `$schema` stamped on each exported entity, one variable per entity kind: `CHAIN_`, `SERVICE_`, `EXTERNAL_SERVICE_`, `INTERNAL_SERVICE_`, `IMPLEMENTED_SERVICE_`, `CONTEXT_SERVICE_`, `MCP_SERVICE_`, `API_`, `API_GROUP_`, `SPECIFICATION_`, `SPECIFICATION_GROUP_`. Override only to match a fork's own schema namespace; the URIs also gate the revert migrations that produce the legacy export. |

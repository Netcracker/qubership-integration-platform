...
| QIP_EXPORT_LEGACY_FORMAT          | false                                                | Exports services under the flat `service-<id>.yaml` name, with the type in `content.integrationSystemType`. Set it when the archive has to be imported by a Runtime Catalog older than 1.0.4.                                                                             |

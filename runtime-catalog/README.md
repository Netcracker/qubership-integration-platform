...
| QIP_EXPORT_LEGACY_FORMAT          | false                                                | Exports services under the flat `service-<id>.yaml` name, with the type in `content.integrationSystemType`. Set it when the archive has to be imported by a Runtime Catalog older than 1.0.4. It carries external, inner cloud, and implemented services only: a context or MCP service gets the flat name `context-service-<id>.yaml` / `mcp-service-<id>.yaml`, which no version of the import discovers. |
| CHAIN_JSON_SCHEMA_URI             | http://qubership.org/schemas/product/qip/chain.schema.yaml | The `$schema` stamped on each exported chain. One variable per exported entity, each defaulting to the matching `<entity>.schema.yaml` URI: `SERVICE_`, `EXTERNAL_SERVICE_`, `INTERNAL_SERVICE_`, `IMPLEMENTED_SERVICE_`, `CONTEXT_SERVICE_`, `MCP_SERVICE_`, `API_`, `API_GROUP_`, `SPECIFICATION_`, and `SPECIFICATION_GROUP_JSON_SCHEMA_URI`. Override them to match a fork's own schema namespace. |

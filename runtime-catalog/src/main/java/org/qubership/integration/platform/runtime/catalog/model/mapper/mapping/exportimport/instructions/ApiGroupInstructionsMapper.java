...
@Component
public class ApiGroupInstructionsMapper implements ImportInstructionsMapper<ImportInstructionsConfig, ImportInstructionsDTO> {
    ...
    public List<ImportInstruction> asEntitiesIncludingDeletes(@NotNull GeneralImportInstructionsConfig generalImportInstructionsConfig) {
        if (generalImportInstructionsConfig.getApiGroups() == null) {
            return Collections.emptyList();
        }

        return configToEntity(
                generalImportInstructionsConfig.getApiGroups().getDelete(),
                ImportEntityType.API_GROUP,
    ...
    public ImportInstructionsConfig asConfig(@NotNull List<ImportInstruction> importInstructions) {
        Set<String> apiGroupsToIgnore = importInstructions.stream()
                .filter(importInstruction -> ImportEntityType.API_GROUP.equals(importInstruction.getEntityType()))
        ...
        return ImportInstructionsConfig.builder()
                .delete(apiGroupsToIgnore)
                .build();
    }

    @Override
    public ImportInstructionsDTO asDTO(@NotNull List<ImportInstruction> importInstructions) {
        Set<ImportInstructionDTO> apiGroupsToIgnore = importInstructions.stream()
                .filter(importInstruction -> ImportEntityType.API_GROUP.equals(importInstruction.getEntityType()))
                .filter(importInstruction -> ImportInstructionAction.DELETE.equals(importInstruction.getAction()))

...
@FieldNameConstants
public class ApiGroup extends AbstractSystemEntity {
    ...
    @OrderBy("id")
    @OneToMany(mappedBy = "apiGroup", fetch = FetchType.LAZY, cascade = CascadeType.ALL)
    ...
    private Set<ApiGroupLabel> labels = new LinkedHashSet<>();
    ...
    public void addSystemModel(SystemModel systemModel) {
        ...
        systemModel.setApiGroup(this);
    }

    public void removeSystemModel(SystemModel systemModel) {
        systemModels.remove(systemModel);
        systemModel.setApiGroup(null);
    }

    public void addLabel(ApiGroupLabel label) {
        this.labels.add(label);
    }

    public void addLabels(Collection<ApiGroupLabel> labels) {
        this.labels.addAll(labels);
    }

    @JsonGetter("labels")
    public Set<ApiGroupLabel> getNonTechnicalLabels() {
    ...
    public void setLabels(Set<ApiGroupLabel> labels) {
        if (this.labels == null) {
    ...
    // Hashes the identity fields plus the url, deliberately leaving out systemModels: equals compares
    // them, but a hash may use a subset and hashing a lazy collection would load it on every lookup.
    @Override
    public int hashCode() {

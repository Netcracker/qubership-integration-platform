...
func TestTheInstallationWideProductionModeIsRead(t *testing.T) {
...
// production. That is the default an unconfigured installation gets.
func TestNoProductionModeAnywhereLeavesTheSettingUnset(t *testing.T) {
	cfg, err := loadConfig(filepath.Join(t.TempDir(), "absent.yaml"))
	require.NoError(t, err)

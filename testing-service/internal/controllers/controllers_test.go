...
// The not-found message opens with the subject, so every read endpoint spells it
// capitalized. The four of them share one helper, and this is what keeps the
// spelling of each attached to its own resource.
func TestFindByIdOpensTheNotFoundMessageWithItsSubject(t *testing.T) {
	noTestCase := func(f *fakes) {
		f.testCases.findByID = func(context.Context, uuid.UUID) (*dao.TestCaseView, error) {
			return nil, nil
		}
	}
	noTestsRun := func(f *fakes) {
		f.testsRuns.findByID = func(context.Context, uuid.UUID) (*dao.TestsRunView, error) {
			return nil, nil
		}
	}
	tests := []struct {
		...
		{"test case", "/test-cases/" + knownID, noTestCase, "Test case " + knownID + " not found."},
		{"tests run", "/tests-runs/" + knownID, noTestsRun, "Tests run " + knownID + " not found."},
		{"test case run", "/test-case-runs/" + knownID, func(*fakes) {}, "Test case run " + knownID + " not found."},
		{"endpoint mock", "/endpoint-mocks/" + knownID, func(*fakes) {}, "Endpoint mock " + knownID + " not found."},
	}
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			f := newFakes()
			...
			response := request(t, app, http.MethodGet, test.target, "")
...
// Ten endpoints answer 204 and nothing else, so a 204 alone proves only that the
// route is mounted. These pin the two halves a shared helper could otherwise
// drop: the service call itself, with the id the path carried, and the failure
// message naming the action that failed.
func TestAnActionOverAnIdRunsTheServiceAndNamesItsFailure(t *testing.T) {
	tests := []struct {
		name    string
		method  string
		target  string
		wire    func(*fakes, func(context.Context, uuid.UUID) error)
		message string
	}{
		{
			name: "delete test case", method: http.MethodDelete, target: "/test-cases/" + knownID,
			wire:    func(f *fakes, act func(context.Context, uuid.UUID) error) { f.testCases.delete = act },
			message: "Failed to delete test case",
		},
		{
			name: "delete endpoint mock", method: http.MethodDelete, target: "/endpoint-mocks/" + knownID,
			wire:    func(f *fakes, act func(context.Context, uuid.UUID) error) { f.endpointMocks.delete = act },
			message: "Failed to delete endpoint mock",
		},
		{
			name: "delete tests run", method: http.MethodDelete, target: "/tests-runs/" + knownID,
			wire:    func(f *fakes, act func(context.Context, uuid.UUID) error) { f.testsRuns.delete = act },
			message: "Failed to delete tests run",
		},
		{
			name: "cancel tests run", method: http.MethodPost, target: "/tests-runs/" + knownID + "/cancel",
			wire:    func(f *fakes, act func(context.Context, uuid.UUID) error) { f.testsRuns.cancel = act },
			message: "Failed to cancel tests run",
		},
		{
			name: "cancel test case run", method: http.MethodPost, target: "/test-case-runs/" + knownID + "/cancel",
			wire:    func(f *fakes, act func(context.Context, uuid.UUID) error) { f.testCaseRuns.cancel = act },
			message: "Failed to cancel test case run",
		},
	}
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			f := newFakes()
			var seen []uuid.UUID
			test.wire(f, func(_ context.Context, id uuid.UUID) error {
				seen = append(seen, id)
				return nil
			})
			app := mount(t, f, config.Config{}, config.Deps{})

			response := request(t, app, test.method, test.target, "")

			require.Equal(t, http.StatusNoContent, response.StatusCode)
			assert.Equal(t, []uuid.UUID{uuid.MustParse(knownID)}, seen)
		})

		t.Run(test.name+" failing", func(t *testing.T) {
			f := newFakes()
			test.wire(f, func(context.Context, uuid.UUID) error {
...
func TestAnActionOverIdsRunsTheServiceAndNamesItsFailure(t *testing.T) {
	tests := []struct {
		name    string
		method  string
		target  string
		...
		{
			name: "bulk delete test cases", method: http.MethodDelete, target: "/test-cases",
			wire:    func(f *fakes, act func(context.Context, *[]uuid.UUID) error) { f.testCases.bulkDelete = act },
			message: "Failed to bulk delete test cases",
		},
		{
			name: "bulk delete endpoint mocks", method: http.MethodDelete, target: "/endpoint-mocks",
			wire:    func(f *fakes, act func(context.Context, *[]uuid.UUID) error) { f.endpointMocks.bulkDelete = act },
			message: "Failed to bulk delete endpoint mocks",
		},
		{
			name: "bulk delete tests runs", method: http.MethodDelete, target: "/tests-runs",
			wire:    func(f *fakes, act func(context.Context, *[]uuid.UUID) error) { f.testsRuns.bulkDelete = act },
			message: "Failed to bulk delete tests runs",
		},
		{
			name: "bulk cancel tests runs", method: http.MethodPost, target: "/tests-runs/cancel",
			wire:    func(f *fakes, act func(context.Context, *[]uuid.UUID) error) { f.testsRuns.bulkCancel = act },
			message: "Failed to bulk cancel tests runs",
		},
		{
			name: "bulk cancel test case runs", method: http.MethodPost, target: "/test-case-runs/cancel",
			wire:    func(f *fakes, act func(context.Context, *[]uuid.UUID) error) { f.testCaseRuns.bulkCancel = act },
			message: "Failed to bulk cancel test case runs",
		},
	}
	body := fmt.Sprintf("[%q]", knownID)
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			f := newFakes()
			var seen [][]uuid.UUID
			test.wire(f, func(_ context.Context, ids *[]uuid.UUID) error {
				seen = append(seen, *ids)
				return nil
			})
			app := mount(t, f, config.Config{}, config.Deps{})

			response := request(t, app, test.method, test.target, body)

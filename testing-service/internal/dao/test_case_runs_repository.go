...
//
// A running case that holds no lease at all is swept as well. Every claim stamps
// a lease in the same statement, so nothing here writes such a row, and the
// branch costs one more disjunct. It is what keeps a row that arrived any other
// way — a hand-edited status, a restore from a schema that predates the lease —
// from being stranded for good: `lease_until < now()` is null rather than true
// for it, and the claim guard then holds its whole test run out of the queue.

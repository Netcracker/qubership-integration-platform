...
var (
	testsRunsSortingFields = []string{"id", "start", "finish", "status", "errors", "test_cases", "created_by", "created_at"}

	testsRunsFilterConfiguration = model.FilterConfiguration{
		...
		"status":     GetEnumFilterConfiguration("tests_run_view.status", RunStatuses),
...
func GetTestsRunsSortingFields() *[]string {
	return &testsRunsSortingFields
}

func GetTestsRunsFilterConfiguration() *model.FilterConfiguration {
	return &testsRunsFilterConfiguration
}
...
func (r *testsRunsRepository) FindAll(
	...
	return listing[TestsRunView]{
		logger:          r.logger,
		paginationLimit: r.paginationLimit,
		subject:         "test runs",
		sortingFields:   GetTestsRunsSortingFields(),
		filters:         GetTestsRunsFilterConfiguration(),
		// The chain id lives two joins away, so only the queries that name it pay
		// for the joins and the resulting DISTINCT.
		decorate: func(query *bun.SelectQuery, specification *model.SelectionSpecification) *bun.SelectQuery {

...
-- ordinal is the order the cases of a run execute in. lease_until and
-- lease_owner are the lease a worker holds over the case it claimed; lease_owner
-- is a fencing token, not a flag, so every write a worker makes about its case is
-- guarded by it and a stalled worker whose lease was swept cannot overwrite the
...
-- What the claim filters and orders on. A plain index on a five-value enum would
-- not help, so status is carried by this one.
...
-- The column list is not spelled out on purpose: PostgreSQL expands
-- test_case_run.* against the table as it stands when the statement runs, so a
-- column added to test_case_runs later reaches this view by re-running the
-- statement. create or replace view can neither drop nor retype a column, and it
-- can only append one at the end, so a column that belongs ahead of the joined
-- ones needs the view dropped and recreated instead.

...
export const AdminToolsSidebar = ({ collapsed }: { collapsed: boolean }) => {
  ...
  const routeOpenKeys = useMemo(
    () => (collapsed ? [] : getOpenKeys(items, location.pathname)),
    [collapsed, items, location.pathname],
  );
  // The open state is controlled rather than seeded through defaultOpenKeys,
  // which is read once at mount — too early for the Testing group, which joins
  // the menu only after its availability resolves. Reopening only when the
  // route moves to another group leaves a group the user closed closed.
  const [openKeys, setOpenKeys] = useState(routeOpenKeys);
  const openedForGroups = useRef(routeOpenKeys.join("|"));

  useEffect(() => {
    const groups = routeOpenKeys.join("|");
    if (groups === openedForGroups.current) {
      return;
    }
    openedForGroups.current = groups;
    setOpenKeys((keys) => [...new Set([...keys, ...routeOpenKeys])]);
  }, [routeOpenKeys]);

...
const ENTITY_TYPES_REQUIRING_NAME = new Set<MatcherEntityType>([
  MatcherEntityType.HEADER,
  MatcherEntityType.PATH_PARAMETER,
  MatcherEntityType.QUERY_PARAMETER,
]);
...
export function matcherRequiresEntityName(
  ...
  return !!entityType && ENTITY_TYPES_REQUIRING_NAME.has(entityType);
}
...
function isAddressablePathParameterName(name: string): boolean {
  return [...name].every(
    (character) =>
      !"/}?#%".includes(character) && (character.codePointAt(0) ?? 0) >= 0x20,
  );
}
...
function parametersKey(
  ...
  return (
    (parameters ?? [])
      ...
      // Ordered by code unit rather than by locale: this is an identity key, and a
      // collation that varies with the locale would let one set of parameters
      // produce two keys.
      .sort(byCodeUnit)
      .join(",")
  );

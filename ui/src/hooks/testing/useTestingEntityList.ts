...
/** How long the search waits for typing to stop before it reaches the service. */
const SEARCH_DEBOUNCE_MS = 300;

/** How long a name lookup is reused before a rename is worth another request. */
export const NAMES_STALE_TIME_MS = 60_000;

/** Selection option that reaches past the loaded page; resolved server-side. */
const SELECT_ALL_MATCHING_KEY = "all-matching";

/** The two orders the table reports, in the spelling the service takes. */
const SORT_ORDER_BY_ANTD_ORDER: Record<"ascend" | "descend", TestingSortOrder> =
  {
    ascend: TestingSortOrder.ASC,
    descend: TestingSortOrder.DESC,
  };
...
export type TestingEntityList<T> = {
  ...
  /** Searches on the text typed so far, for the Enter key and the search button. */
  confirmSearch: () => void;
};
...
/** One entry per scope: the chains of the installation, or the elements of one chain. */
function namesQueryKey(chainId: string | undefined) {
  return ["testing", "names", chainId ?? null];
}
...
async function loadNames(chainId: string | undefined): Promise<NameLookup> {
  if (chainId) {
    return {
      chains: NO_NAMES,
 ...
 *
 * Names come from one cached request each — the chains of the whole
 * installation, or the elements of the chain in scope. There is no cross-chain
 * element lookup, so an element of another chain keeps its id. The cache is
 * keyed by the scope, so a filter written against a name waits for the names of
 * the chain now in context rather than resolving into the ids of the one before it.
 */
export function useTestingEntityList<T extends { id: string }>({
  ...
  // Cached past a tab switch and past a remount, and keyed by the scope: the
  // names of the chain left behind are dropped the moment the scope changes
  // rather than when the names of the new one arrive. The window is finite
  // because a chain or an element can be renamed while the tab is open, and
  // caching for the session would leave every testing list on the old name.
  // The shared client turns mount refetching off, so without the override below
  // the window would pass and nothing would ever read the names again.
  const {
    data: names,
    isError: queryFailed,
    error: namesError,
  } = useQuery<NameLookup>({
    queryKey: namesQueryKey(chainId),
    queryFn: () => loadNames(chainId),
    enabled: usesNames,
    retry: false,
    staleTime: NAMES_STALE_TIME_MS,
    refetchOnWindowFocus: false,
    refetchOnMount: true,
  });

  // The cache entry is shared with the lists that do read names, so a failure of
  // theirs is not one of ours. A refresh that fails leaves the names read before
  // it in place, and those still name the rows a filter is written against.
  const namesFailed = usesNames && queryFailed && !names;

  useEffect(() => {
    if (namesFailed) {
      notificationService.requestFailed("Failed to resolve names", namesError);
    }
  }, [namesFailed, namesError, notificationService]);
  ...
  const holdingForNames =
    filtersNeedNames && usesNames && !names && !namesFailed;
  const unresolvableNames =
    filtersNeedNames && (holdingForNames || namesFailed);
  ...
  }, [unresolvableNames, selection, scopeFilters, chainId, debouncedSearch]);
  ...
  // Sorting, filtering and paging can still put several requests in flight at
  // once. Only the newest may write, which also keeps a resolved request from
  // writing after the screen is gone.

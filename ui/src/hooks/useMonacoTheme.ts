...
export function useMonacoEditorOptions() {
  ...
  useEffect(() => {
    ...
    const fontSize =
      parseInt(style.getPropertyValue("--vscode-editor-font-size").trim()) ||
      13;
    const lineHeight =
      parseInt(style.getPropertyValue("--vscode-editor-line-height").trim()) ||
      20;
    const fontWeight =
      style.getPropertyValue("--vscode-editor-font-weight").trim() || "normal";
...
export function applyVSCodeThemeToMonaco(
  ...
  const applyTheme = () => {
    ...
    const isDark =
      themeData?.isDark ??
      (dataTheme === "dark" || dataTheme === "high-contrast");
    ...
    const editorBg = getColorFromThemeOrCss(
      "editor.background",
      "--vscode-editor-background",
      isDark ? "#141414" : "#ffffff",
    );
    const editorFg = getColorFromThemeOrCss(
      "editor.foreground",
    ...
    const selection = getColorFromThemeOrCss(

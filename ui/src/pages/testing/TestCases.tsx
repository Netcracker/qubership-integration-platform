...
export const TestCases: React.FC<TestCasesProps> = ({
  ...
  const deleteSelected = useCallback(async () => {
    try {
      ...
      await api.deleteTestCases(ids);
      clearSelection();
      refresh();
    } catch (error) {
      notificationService.requestFailed("Failed to delete test cases", error);
    }
  ...
  const handleDelete = useCallback(() => {
    ...
    const target = selectAllMatching
      ? "all test cases that match the filters"
      : `${selectedRowKeys.length} test case${selectedRowKeys.length === 1 ? "" : "s"}`;
    confirmAndRun({
  ...
  const handleTableChange = useCallback<
    NonNullable<TableProps<TestCaseView>["onChange"]>
  ...
  const rowSelection: TableRowSelection<TestCaseView> = {
    type: "checkbox",
    selectedRowKeys,
    onChange: (keys) => {
    ...
    () => (
      <>
        <ProtectedButton
          ...
          buttonProps={{
            "data-testid": "test-cases-refresh",
            iconName: "refresh",
        ...
        <ProtectedButton
          ...
          tooltipProps={{ title: "Run selected test cases" }}
          buttonProps={{
        ...
        <ProtectedButton
          ...
          tooltipProps={{ title: "Export selected test cases" }}
          buttonProps={{
        ...
        {chainId ? null : (
          <ProtectedButton
            ...
            tooltipProps={{ title: "Import test cases" }}
            buttonProps={{
              "data-testid": "test-cases-import",
        ...
        <ProtectedButton
          ...
          tooltipProps={{ title: "Delete selected test cases" }}
          buttonProps={{
        ...
        {chainId ? (
          <ProtectedButton
            require={permissions.write}
            tooltipProps={{ title: "Create a test case" }}
            buttonProps={{
              "data-testid": "test-cases-create",
              type: "primary",
              iconName: "plus",
              children: "Create",

...
jest.mock("../../../src/components/table/CompactSearch.tsx", () => ({
  CompactSearch: (props: {
    ...
    <input
      ...
      // Enter is what the real component submits on; it adds no node to the DOM.
      onKeyDown={(event) => {
        if (event.key === "Enter") props.onSearchConfirm?.(props.value);
      }}
    />
  ),
}));
...
describe("TestCases selection lifetime", () => {
  ...
  it("should search without waiting out the debounce when Enter confirms it", async () => {
    mockGetTestCases.mockResolvedValue(twoCases);
    renderTestCases();
    await screen.findByText("Second case");
    const asked = mockGetTestCases.mock.calls.length;
    ...
    expect(mockGetTestCases).toHaveBeenCalledTimes(asked + 1);
    expect(mockGetTestCases).toHaveBeenLastCalledWith(
      expect.objectContaining({ searchText: "second" }),
      expect.anything(),
    );
  });

...
export class SpecificationImportApiHandler {
  ...
  async handleImportSpecificationGroup(
    ...
    try {
      ...
      console.log(
        `[SpecificationImportApiHandler] Import completed successfully:`,
        {
          importId: result.id,
  ...
  async handleImportSpecification(
    specificationGroupId: string,
    files: SerializedFile[],
    systemId: string,
  ): Promise<ImportSpecificationResult> {
    console.log(
      `[SpecificationImportApiHandler] Handling import specification request`,
    );
    ...
    try {
      const result = await this.service.importSpecification(
        specificationGroupId,
        files,
        systemId,
      );
      console.log(
        `[SpecificationImportApiHandler] Specification import completed successfully:`,
        {
          importId: result.id,
          specificationGroupId: result.specificationGroupId,
          done: result.done,
      ...
      console.error(
        `[SpecificationImportApiHandler] Specification import failed:`,
        error,
      );
      throw error;
    }
  ...
  async handleGetImportResult(
    importId: string,
  ): Promise<ImportSpecificationResult> {
    console.log(
      `[SpecificationImportApiHandler] Getting import result for ID: ${importId}`,
    );
    ...
    try {
      ...
      console.error(
        `[SpecificationImportApiHandler] Failed to get import result:`,
        error,
      );
      throw error;
    }

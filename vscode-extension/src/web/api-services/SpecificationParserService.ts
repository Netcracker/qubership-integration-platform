...
import type {
  ProtoData,
  ResolvedProtoOperation,
} from "./parsers/proto/ProtoTypes";
...
export class SpecificationParserService {
  ...
  async parseSpecification(file: SerializedFile): Promise<ParsedSpecification> {
    try {
      ...
      return this.createErrorSpecification(
        file,
  ...
  private detectSpecificationType(file: SerializedFile): ApiSpecificationType {
    const extension = FileConversionService.getFileExtension(
      file.name,
    ).toLowerCase();
   ...
   */
  private async parseOpenApiSpecification(
    content: string,
    file: SerializedFile,
  ): Promise<ParsedSpecification> {
    try {
      ...
      return {
        ...
        name:
          spec.info?.title ||
          FileConversionService.getFileNameWithoutExtension(file.name),
   ...
   */
  private async parseAsyncApiSpecification(
    content: string,
    file: SerializedFile,
  ): Promise<ParsedSpecification> {
    try {
          ...
          if (typeof channel === "object" && channel !== null) {
            ...
            if (ch.subscribe) {
              operations.push({
                ...
                parameters: this.parseAsyncApiParameters(
                  ch.subscribe.parameters || {},
                ),
      ...
      throw new Error(
        `Failed to parse AsyncAPI specification: ${error instanceof Error ? error.message : "Unknown error"}`,
      );
    }
  }
   ...
   */
  private async parseGraphQLSpecification(
    content: string,
    file: SerializedFile,
  ): Promise<ParsedSpecification> {
    try {
      ...
      throw new Error(
        `Failed to parse GraphQL specification: ${error instanceof Error ? error.message : "Unknown error"}`,
      );
    }
  }
   ...
   */
  private async parseGrpcSpecification(
    content: string,
    file: SerializedFile,
  ): Promise<ParsedSpecification> {
    try {
      ...
      throw new Error(
        `Failed to parse gRPC specification: ${error instanceof Error ? error.message : "Unknown error"}`,
      );
    }
  }

  private buildParsedGrpcOperation(
    ...
    return {
      ...
      rpcType: this.resolveGrpcCallType(
        operation.requestStream,
        operation.responseStream,
      ),
      metadata: {
  ...
  private resolveGrpcCallType(
    requestStream: boolean,
    responseStream: boolean,
  ): ParsedOperation["rpcType"] {
   ...
   */
  private async parseSoapSpecification(
    content: string,
    file: SerializedFile,
  ): Promise<ParsedSpecification> {
    try {
      ...
      const portTypeMatch = content.match(
        /<portType[^>]*name="([^"]*)"[^>]*>(.*?)<\/portType>/s,
      );
      if (portTypeMatch) {
        const serviceName = portTypeMatch[1];
        const portTypeContent = portTypeMatch[2];

        const operationMatches = portTypeContent.match(
          /<operation[^>]*name="([^"]*)"[^>]*>/g,
        );
        if (operationMatches) {
      ...
      throw new Error(
        `Failed to parse SOAP specification: ${error instanceof Error ? error.message : "Unknown error"}`,
      );
    }
  }
  ...
  private parseOpenApiResponses(responses: any): ParsedResponse[] {
    return Object.entries(responses).map(([statusCode, response]) => ({
      ...
      contentType: (response as any).content
        ? Object.keys((response as any).content)[0]
        : undefined,
      schema: (response as any).content
        ? Object.values((response as any).content)[0]
        : undefined,
    }));
  }
   ...
   */
  private createErrorSpecification(
    file: SerializedFile,
    error: string,
  ): ParsedSpecification {
    return {

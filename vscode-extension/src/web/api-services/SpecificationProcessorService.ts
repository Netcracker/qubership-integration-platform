...
export class SpecificationProcessorService {
  ...
  private async processSpecificationFile(
    ...
    if (!specificationType) {
      if (this.isWsdlDependencyExtension(fileExtension)) {
        console.log(
          "[SpecificationProcessorService] Skipping WSDL dependency file",
          file.name,
        );
        return;
      }
    ...
    const specificationId = this.buildSpecificationId(
      systemId,
      specificationGroup.name,
      version,
    );
    const operations = await this.createOperationsFromFile(
      file,
      specificationType,
      specificationId,
      allFiles,
    ...
    if (
      ...
      throw new Error(
        `No gRPC operations detected in ${file.name}. Ensure the proto file declares at least one RPC method.`,
      );
    }
    console.log(
      "[SpecificationProcessorService] Generated",
      operations.length,
      "operations for",
      file.name,
    );
    const specification = this.createSpecificationEntity(
      file,
  ...
  private buildSpecificationId(
    systemId: string | undefined,
    groupName: string,
    version: string,
  ): string {
    return systemId
      ? `${systemId}-${groupName}-${version}`
      : crypto.randomUUID();
  }
  ...
  detectProtocolFromSpecification(specData: any): string | null {
    ...
    if (specData.asyncapi) {
      const protocol =
        specData.info?.["x-protocol"]?.toLowerCase() ||
  ...
  extractEnvironmentCandidates(specData: any): EnvironmentCandidate[] {
    ...
    if (specData.asyncapi) {
      ...
      if (specData.servers && typeof specData.servers === "object") {
        Object.entries(specData.servers).forEach(
      ...
      if (candidates.length === initialCandidateCount) {
        const fallbackAddress =
          AsyncApiSpecificationParser.extractAddressFromAsyncApiData(specData);
        const normalizedAddress = this.normalizeEnvironmentAddress(
  ...
  private async buildWsdlAdditionalDocuments(
    ...
    if (documents.length === 0) {
      console.log(
        "[SpecificationProcessorService] No WSDL dependencies detected for",
        mainPath,
      );
    } else {
      console.log(
        "[SpecificationProcessorService] Resolved WSDL dependencies for",
        mainPath,
  ...
  private extractOpenApiServers(specData: any): EnvironmentCandidate[] {
    ...
    if (Array.isArray(specData.servers) && specData.servers.length > 0) {
      specData.servers.forEach((server: any, index: number) => {
        ...
        if (normalizedAddress) {
          ...
          candidates.push({
            name,
            address: normalizedAddress,
            protocol: this.resolveProtocolName(
              server?.protocol ||
                specData.info?.["x-protocol"] ||
                specData.protocol,
            ),
      ...
      const schemes =
        Array.isArray(specData.schemes) && specData.schemes.length > 0
          ? specData.schemes
          : ["https"];
      const basePath =
        typeof specData.basePath === "string" ? specData.basePath : "";
  ...
  private deduplicateEnvironmentCandidates(
  ...
  private async createOperationsFromFile(
    ...
    try {
      ...
      if (!content) {
        throw new Error(
          `Unable to read specification content from ${file.name}`,
        );
      }
      ...
      switch (actualSpecificationType) {
        case ApiSpecificationType.SOAP:
          const additionalDocuments = await this.buildWsdlAdditionalDocuments(
            file,
            allFiles,
            contentCache,
          );
          const wsdlData = await SoapSpecificationParser.parseWsdlContent(
            content,
            {
              fileName: file.name,
              additionalDocuments,
          ...
          return SoapSpecificationParser.createOperationsFromWsdl(
            wsdlData,
            specificationId,
          );
        ...
        case ApiSpecificationType.GRPC:
          const protoData =
            await ProtoSpecificationParser.parseProtoContent(content);
        ...
        case ApiSpecificationType.GRAPHQL:
          const graphqlData =
            await GraphQLSpecificationParser.parseGraphQLContent(content);
          return GraphQLSpecificationParser.createOperationsFromGraphQL(
            graphqlData,
            specificationId,
          );
        ...
        default:
          throw new Error(
            `Unsupported specification type ${specificationType} for ${file.name}`,
          );
      }
    } catch (error) {
      console.error(
        "[SpecificationProcessorService] Failed to create operations:",
        error,
      );
      throw error instanceof Error
        ? error
        : new Error("Failed to create operations");
    }
  }

  private detectTypeByExtension(
    extension: string,

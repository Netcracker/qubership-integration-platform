...
export class GraphQLSpecificationParser {
  static async parseGraphQLContent(content: string): Promise<GraphQLData> {
    ...
    const mutationTypeName = this.resolveOperationTypeName(
      document,
      "mutation",
    );

    const { queries, mutations } = this.collectOperations(
      document,
      queryTypeName,
      mutationTypeName,
    );
    const types = this.collectObjectTypes(document, [
      queryTypeName,
      mutationTypeName,
    ]);
  ...
  static createOperationsFromGraphQL(
    ...
    for (const mutation of graphqlData.mutations) {
      operations.push(
        this.buildOperation(specificationId, "mutation", mutation),
      );
    }
  ...
  private static resolveOperationTypeName(
    ...
    if (schemaDef) {
      const mapping = schemaDef.operationTypes.find(
    ...
    const typeDef = document.definitions.find(
      (definition) =>
        ...
        definition.name.value === fallbackName,
    );
  ...
  private static mapFieldsToOperations(
  ...
  private static collectObjectTypes(
    document: DocumentNode,

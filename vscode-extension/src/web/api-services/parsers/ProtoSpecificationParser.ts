...
} from "./proto/ProtoOperationResolver";
...
export class ProtoSpecificationParser {
  static async parseProtoContent(content: string): Promise<ProtoData> {
    ...
    if (services.length === 0) {
      throw new Error(
        "Proto specification must declare at least one service definition",
      );
    }

    const hasRpcMethods = services.some(
      (service) => service.methods.length > 0,
    );
    if (!hasRpcMethods) {
      throw new Error(
        "Proto specification must declare at least one RPC method",
      );
    }
  ...
  static createOperationsFromProto(
    protoData: ProtoData,
    ...
    while (queue.length > 0) {
      ...
      for (const element of nested) {
        if (element instanceof protobuf.Service) {
          ...
          element instanceof protobuf.Namespace ||
          element instanceof protobuf.Type
        ) {
          queue.push(element);
        }
      }
  ...
  private static convertMethod(
    ...
    const requestType = resolveMethodType(
      method.resolvedRequestType,
      method.requestType,
      servicePackage,
    );
    const responseType = resolveMethodType(
...
function resolveMethodType(
  ...
  if (
    resolved &&
    "fullName" in resolved &&
    typeof resolved.fullName === "string"
  ) {

...
export async function getServiceSpecificationsUri(
  serviceFileUri: vscode.Uri,
  groupId: string,
): Promise<string> {
...
export async function getServiceOperationsUri(
  serviceFileUri: vscode.Uri,
  groupId: string,
  specId: string,
): Promise<string> {
...
export async function handleImportSpecificationGroup(
...
export async function handleImportSpecification(
  context: ExtensionContext | undefined,
  serviceFileUri: vscode.Uri,
  payload: any,
): Promise<any> {
  ...
  return await importSpecHandler.handleImportSpecification(
    payload.specificationGroupId,
    payload.files as SerializedFile[],
    payload.systemId,
  );
}

export async function handleGetImportSpecificationResult(
  context: ExtensionContext | undefined,
  serviceFileUri: vscode.Uri,
  payload: any,
): Promise<any> {
...
export async function getBaseFolder(
  ...
  if (fileOrFolder) {
    ...
    if (type === QipFileType.SERVICE || type === QipFileType.CHAIN) {
      ...
      const parentPath =
        lastSlashIndex > 0
          ? fileOrFolder.path.substring(0, lastSlashIndex)
          : fileOrFolder.path;
...
export async function handleGetSpecApiFiles(): Promise<any> {
  ...
  for (const fileUri of apiFileUris) {
    try {
      ...
      console.error(
        `[handleGetSpecApiFiles] Failed to process ${fileUri.path}`,
        error,
      );
    }
  }
...
export async function handleReadSpecificationFileContent(
  fileUri: string,
  specificationFilePath: string,
): Promise<string> {
  const apiFileUri = vscode.Uri.parse(fileUri);
  const apiFileDir = apiFileUri.with({
    path: apiFileUri.path.substring(0, apiFileUri.path.lastIndexOf("/")),
  });
  ...
  try {
    ...
    if (!specificationFilePath.includes("resources/")) {
      ...
      try {
        ...
        throw new Error(
          `Specification file not found at ${specificationFilePath} or ${resourcesPath}`,
        );
      }
    }
    throw error;

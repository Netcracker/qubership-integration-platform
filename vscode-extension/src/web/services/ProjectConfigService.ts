...
export class ProjectConfigService {
  ...
  async setCurrentContext(appName: string, workspaceUri?: Uri): Promise<void> {
    console.log(
      `[ProjectConfigService] setCurrentContext called: appName="${appName}", current="${this.currentAppName}"`,
    );

    if (this.shouldSkipContextSwitch(appName)) {
      console.log(
        `[ProjectConfigService] Skipping context switch - external config is active`,
      );
      return;
    }
    ...
    console.log(
      `[ProjectConfigService] Context switched: appName="${this.currentAppName}"`,
    );
  }
  ...
  async loadConfigFromUri(configUri: Uri): Promise<void> {
    ...
    if (!configFile) {
      console.warn(
        "[ProjectConfigService] Failed to load external config from",
        configUri.toString(),
      );
      return;
    }
    ...
    for (const [appName, configData] of Object.entries(configFile.configs)) {
      const config = this.buildConfigFromData(
        appName,
        configData,
        configFile.version,
        configFile.cache,
    ...
    console.log(
      "[ProjectConfigService] External config loaded from",
      configUri.toString(),
    );
  }

  private async loadConfigFileFromUri(
    configUri: Uri,
    ...
    try {
      ...
      if (!configFile || !configFile.configs) {
        console.warn(
          "[ProjectConfigService] Invalid config structure at",
          configUri.toString(),
        );
        return null;
      }
      ...
      console.warn(
        "[ProjectConfigService] Error reading config from",
        configUri.toString(),
        error,
      );
      return null;
  ...
  private addExternalConfig(appName: string, config: ProjectConfig): void {
    ...
    console.log(
      `[ProjectConfigService] External config added: appName="${appName}", extensions:`,
      config.extensions,
    );
  }

  private updateCurrentConfigIfNeeded(
    appName: string,
    config: ProjectConfig,
    isFirstExternalConfig: boolean,
  ): void {
    if (this.shouldUpdateCurrentConfig(appName, isFirstExternalConfig)) {
      ...
      if (wasAutoSwitched) {
        console.log(
          `[ProjectConfigService] Auto-switched to external config: appName="${appName}"`,
        );
      }
    }
  }

  private shouldUpdateCurrentConfig(
    appName: string,
    isFirstExternalConfig: boolean,
  ): boolean {
  ...
  private shouldSkipContextSwitch(requestedAppName: string): boolean {
    ...
    return (
      hasExternalConfig && isAutoCallWithDefault && alreadySwitchedToExternal
    );
  }

  private parseConfigFile(
    configFile: ProjectConfigFile,
    targetMap: Map<string, ProjectConfig>,
  ): void {
    for (const [appName, configData] of Object.entries(configFile.configs)) {
      const config = this.buildConfigFromData(
        appName,
      ...
      console.log(
        `[ProjectConfigService] Config parsed: appName="${appName}", extensions:`,
        config.extensions,
      );
    }
  }
  ...
  unregisterExternalConfig(appName: string): void {
    ...
    console.log(
      `[ProjectConfigService] External config unregistered: appName="${appName}"`,
    );
  }
  ...
  async loadWorkspaceConfig(workspaceUri: Uri): Promise<void> {
    ...
    if (configFile) {
      ...
      console.log(
        "[ProjectConfigService] No workspace config found, using embedded and external configs",
      );
    }

    console.log(
      `[ProjectConfigService] Total configs loaded: ${this.workspaceConfigs.size}`,
    );
  }
  ...
  private loadWorkspaceConfigsFromFile(configFile: ProjectConfigFile): void {
    for (const [appName, configData] of Object.entries(configFile.configs)) {
      ...
      console.log(
        `[ProjectConfigService] Workspace config loaded: appName="${appName}", extensions:`,
        config.extensions,
      );
    }
  }
  ...
  async updateExtension(
    ...
    await this.updateConfigProperty(
      "extensions",
      type,
      extension,
      workspaceUri,
